...
const themeEntries = (() => {
    ...
    const globPath = path.join(__dirname, "js", "assets", "themes", "*");
    ...
    return files.filter((f) => f.lastIndexOf('/') === f.length - 1).reduce((res, curr) => {
        ...
        finalRes["themes/" + path.basename(curr, path.extname(curr))] = path.join(__dirname, "js", "assets", "themes", `${path.basename(curr, path.extname(curr))}`, "theme.less");
        return finalRes;
    }, {});
